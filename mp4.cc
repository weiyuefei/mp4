...
{
  const char *method, *path, *start_time;
  int method_len, path_len, start_time_len;
  int ret;
  float start;
  TSMLoc ae_field, range_field, start_time_field;
  TSCont contp;
  Mp4Context *mc;
  ...
  //如果头里带这Start-Time 这当作一般的http 请求
  start_time_field = TSMimeHdrFieldFind(rri->requestBufp, rri->requestHdrp,"Start-Time", 10);
  if (start_time_field) {
	start_time = TSMimeHdrFieldValueStringGet(rri->requestBufp, rri->requestHdrp,start_time_field, -1, &start_time_len);
	if (start_time != NULL) {
		ret = sscanf(start_time, "%f", &start);
		if (ret != 1)
			start = 0;
	}
  }
